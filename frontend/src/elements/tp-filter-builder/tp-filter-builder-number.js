...
class TpFilterBuilderNumber extends FilterBuilderField(LitElement) {
  static get styles() {
    return [
      shared,
      css`
      ...
      <tp-form>
        <form class="wrap">
          ...
          <div class="remove" @click=${this._removeFilterField}>
            <tp-icon .icon=${TpFilterBuilderNumber.iconRemove} tooltip="Remove this filter"></tp-icon>
          </div>
        </form>
      </tp-form>
    `;
...
window.customElements.define('tp-filter-builder-number', TpFilterBuilderNumber);